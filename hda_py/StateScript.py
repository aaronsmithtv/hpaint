        ...
        def project_point(self, node, mouse_point, mouse_dir, intersect_geometry):
                ...
                if proj_type > 3:
                        if intersect_geometry is not None:
                                ...
                                prim_num = intersect_geometry.intersect(mouse_point, mouse_dir, hit_point_geo, normal, uvw, None, 0, 1e18, 5e-3)
                                if prim_num >= 0:
...
class State(object):
                ...
                with hou.undos.group("Draw Stroke"):
                        ...
                        for (mirror, mirrorData) in zip(mirrorlist, self.strokesMirrorData):
                                ...
                                for i in range(self.strokesNextToEncode, len(self.strokes)):
                                        ...
                                        (mirroredstroke.proj_pos, _, mirroredstroke.proj_uv, mirroredstroke.proj_prim, mirroredstroke.proj_success) = self.cursor_adv.project_point(node, mirroredstroke.pos, mirroredstroke.dir, self.intersectGeometry(node))
        ...
        def stroke_from_event(self, ui_event, device, node):
                ...
                (sdata.proj_pos, _, sdata.proj_uv, sdata.proj_prim, sdata.proj_success) = self.cursor_adv.project_point(node, sdata.pos, sdata.dir, self.intersectGeometry(node))
                return sdata

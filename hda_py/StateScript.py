...
class StrokeParams(object):
    ...
    def __init__(self, node, inst):
        ...
        def valid_parm(parm):
            return parm.isMultiParmInstance() and parm.name().startswith(param_name)
...
class StrokeData(object):
    ...
    def encode(self):
        ...
        # stream.add(self.normal, hou.Vector3)
        return stream
...
class StrokeCursorAdv(object):
    ...
    @staticmethod
    def project_point(node, mouse_point, mouse_dir, intersect_geometry):
        """
...
class State(object):
    ...
    @staticmethod
    def activeMirrorTransforms(node):
    ...
    def stroke_interactive_mask(self, ui_event, node, captured_parms):
        """ The logic for drawing a stroke, opening/closing undo
                blocks, and assigning prestroke / poststroke callbacks.
    ...
    def eraser_interactive(self, ui_event, node, captured_parms):
        """ The logic for erasing as a stroke, and opening an
                eraser-specific undo block.
    ...
    def apply_stroke(self, node, ui_event, update, captured_parms):
        ...
        with hou.undos.group("Draw Stroke"):
            ...
            extra_mirrors = len(mirrorlist) - len(self.strokesMirrorData)
            if extra_mirrors > 0:
    ...
    def stroke_from_event(self, ui_event, device, node):
        ...
        (sdata.proj_pos,
         ...
         sdata.proj_uv,
         sdata.proj_prim,
         sdata.proj_success) = self.cursor_adv.project_point(
            node,
            sdata.pos,
            sdata.dir,
            self.intersectGeometry(node)
    ...
    def cache_strokes(self, node):
        """Used with post-stroke callback, take the stroke that
           was just drawn and cache it into hpaint's data parameter
    ...
    def add_stroke_num(self, node):
        """Add to the internal stroke counter on HDA
           used for group IDs
    ...
    def update_eraser(self, ui_event, node):
        """Turn on the eraser when ctrl is pressed
           uses eraser_enabled to bool eraser on
    ...
    @staticmethod
    def generate_text_drawable(scene_viewer):
        """     Generate all of the parameters used with the Hpaint
                text drawable.
    ...
    def set_max_strokes_global(self, node, input_geo):
        """ Used when strokes are cached, saves the current
                HDA stroke counter value as a global on outgoing
                strokes.
    ...
    @staticmethod
    def set_global_attrib(input_geo, attrib_name, value, default_value):
    ...
    @staticmethod
    def shift_surface_dist(node, direction_id):
        """ Use this to change the Stroke Surface Distance parameter
                when the brackets are pressed. Each bracket press gives the
                specified shift in one direction
    ...
    def undoblock_open(self, block_name):
        """ Open up an undo block safely without chance
                of a conflict
        """
    ...
    def undoblock_close(self):
        """ Close the active undo block and prevent
                a new undo block from being generated
        """
        if self.undo_state == 0:
            return
        elif self.undo_state:
